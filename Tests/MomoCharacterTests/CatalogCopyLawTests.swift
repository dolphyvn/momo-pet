...
/// on Home. A line longer than 12 whitespace-separated words fails with the
/// key attributed. The word COUNT is measured strictly (whitespace tokens —
/// punctuation and the ellipsis are not words), so the pin cannot be gamed
/// by joining with commas.
@Suite("Catalog copy law — 12-word max over the visual body-copy classes (TASK-033)")
struct CatalogCopyLawTests {

    /// The scanned classes: the four time slots (ten lines each), the moment
    /// placeholder (same visual class), and the greetings (disclosed
    /// strict — see the header).
    private static let scannedPattern =
        "^momo\\.line\\.(morning|day|evening|night|moment)\\.\\d{2}$"
    ...
    @Test("every visual body-copy line and greeting stays within the 12-word hard max")
    func twelveWordMaxOverVisualClasses() throws {
        ...
        // placeholder + the 3 greetings are all in scope.
        #expect(scanned == 44, "expected 44 scanned lines (40 slots + 1 moment + 3 greetings); found \(scanned) — a new visual class needs this law's scope review")
