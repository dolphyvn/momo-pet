...
Codec discipline follows the TASK-040 reset-marker adjudication exactly: hand-written Codable with `decodeIfPresent`/`encodeIfPresent`, **no `schemaVersion` bump**, OBS-3 parity-fixture obligation NOT fired. A snapshot decoding without `character` (cross-version skew — cannot ship in Phase 1, both targets update together) degrades honestly: words + quest line render, the pet canvas slot is HELD at its fixed height with the palette blanket (no layout jump at the degradation or on recovery — the pinned W1 behavior), DEBUG-loud log; never a crash, never an error surface (UX §9). (Wording reconciled in TASK-042 — REVIEW-TASK-041 F-2: the original "the pet canvas slot is skipped" contradicted the shipped held-slot behavior.)
