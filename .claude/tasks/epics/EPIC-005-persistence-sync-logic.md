...
## Status
IN_PROGRESS (3/4) — branch `feature/EPIC-005-persistence` cut from `main` @ `04d07d6` (the EPIC-004 merge).
...
- **TASK-023 DONE** — commit `9109457` (pushed). Sync pure halves (05 §6.2/§6.4/§6.6, ADR-003, INV-10; NO transport — EPIC-008): `WatchSnapshot`/`IntentEvent` versioned DTOs (§6.2 sketch verbatim, wrapping `InteractionIntent`/`[QuestProgress]`; hand-written Codable + field-wise Equatable DISCLOSED — MomoCore freeze; exhaustive case-name maps compile-pinned), `IntentJournal` (NDJSON, torn-line tolerance + seal-before-append, per-event epoch, epoch-matched `≤` prune via atomic rewrite, empty-survivor removal with exists-guard), `SyncState` (per-epoch watermark table, THE 0-init accessor, max-semantics recording, monotone seq consumption, accessor-driven `shouldApply` wrapper), `WatchSyncGate` (UUID-first + strict `>` formula), `WatchSnapshotBuilder` (ambient-free, watermark-pair coherent from one epoch input), `SyncStateStore` (plain-JSON, garbled→fresh, **replace-when-present/move-when-absent commit point**), 6 authority-labeled StoreRules constants. `swift test` **515/54 green** (baseline 451/49). Cycle events: orchestrator verification caught 1 MAJOR (SyncStateStore second-save Code-516 — `moveItem` cannot replace; the contract's re-persistence pin only exercised a first save; fixed in §11 fix-loop round 1 + 2 structural pins incl. a DEBUG crash-trap); REVIEW-TASK-023 **APPROVED_WITH_MINOR_NOTES** (0 MAJOR/0 MINOR; derivation-before-code, 4/4 mutations bit, hash-proven restores re-verified). Disposition §8: NITPICK-1 fixed (exists-guard refinement — a bare assertion would false-positive on the pinned absent-journal case), O2 accepted as the doc-faithful INV-10 deviation (exactly-once under FIFO / at-most-once under seeded shuffles — a full-application-under-shuffle pin would pin a falsehood against §6.4's `>` formula), O1+O5 routed to EPIC-008 (single-writer-per-file serialization; call `SyncState.shouldApply`, never resolve watermarks at call sites). MomoCore diff EMPTY throughout.
- Next: TASK-024 MomoKit suite audit + ≥ 80 % coverage floor (05 §10.2/§10.4; delivery-plan row) — the EPIC-005 closer, then epic merge to `main` per §14.
