...
## Status
IN_PROGRESS (3/4) — TASK-008 DONE (`172bc11`; ADR-008 pins; review chain REVIEW-TASK-008 → fix pass → VERIFY all clean). TASK-009 DONE (`b28ccb4`; package + app targets + placeholder shells; REVIEW-TASK-009 APPROVED 0/0/3; build baseline green, swift-test hostability resolved). TASK-010 DONE (`a9b9993`; five-target test harness + import-whitelist + banned-vocabulary scans; REVIEW-TASK-010 APPROVED 0/0/2, both mechanical; swift test 20/5 suites, both UI-test schemes green on pinned sims). TASK-011 TODO pending dispatch — last task of the epic.
