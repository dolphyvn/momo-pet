...
## Status
**IN_PROGRESS (1/9)** — epic authored 2026-09-10 on `feature/EPIC-007-iphone-home` (from merged `main` @ `13a75bf`; the contract commit itself entered `main` via the owner's PR #9 merge `74eba4f`). **TASK-031 DONE** (`257a58f`, REVIEW-TASK-031 APPROVED_WITH_MINOR_NOTES, 851/85 green, MomoKit 91.42 %) — the facade is live behind the placeholder shell. Next: TASK-032 (onboarding over `requiresOnboarding` + the delivery seam) → TASK-033 (Home) → TASK-034/035/036 → TASK-037/038 → TASK-039 (consolidation + audit) → §14 merge.
