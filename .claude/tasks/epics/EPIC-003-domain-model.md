...
## Status
**DONE (2/2) and merged to `main`** (`4291f02`, direct merge per the §14 owner rule; the owner's PR #4 had earlier integrated the branch's docs-only prefix through `8ae4970`). TASK-012 (`bc95cb9`, REVIEW-TASK-012 APPROVED_WITH_MINOR_NOTES 0/1/3) · TASK-013 (`5bbdd72`, REVIEW-TASK-013 CHANGES_REQUIRED → fix round 1 → REVISED APPROVED_WITH_MINOR_NOTES; Probe D now fails the build by design). `swift test` 80 tests / 14 suites green (1203 sweep cases); merged `main` verified green.
