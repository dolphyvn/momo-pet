...
> **Clarification — Q6-credit reading (REVIEW-TASK-018, open question 1, 2026-09-09):** "the rolling 3-day window lacks Q6" above means **both** prior sets lack Q6 — a new day earns Q6 credit iff **both** priors carry Q6 (`count == 2`); unknown or missing priors count as no credit and force Q6 (hence fresh-install day 1 always includes Q6). This strong reading is normative: the "≥ 4 pairs in the tightest case" arithmetic above (Q6-restriction active while yesterday's pair carries Q6 and the older prior lacks it) is coherent and reachable only under it. PRD §5.3's window invariant holds under either reading, so the invariant is not the discriminator — the non-emptiness arithmetic is.
...
- **OBS-D resolution (REVIEW-TASK-019, 2026-09-09):** the "exactly three" namespaces above govern the **variational selection** classes. The fixed state vocabulary resolved for `DisplayState` (04 §3.5's mood word, energy phrase, bond descriptor — the `*Key` fields of §4.11) lives in a fourth keyspace, `momo.line.vocab.<field>.<band>` (e.g. `momo.line.vocab.mood.wistful`), under 04 §8.4's dot convention. It is a **fixed lookup, not a selection** — zero variation, keys named by band (any display-word remap, e.g. Wistful → "quiet", lives in the catalog entry, never the key) — so the day-stable seeded pick / epoch machinery above rightly does not apply to it.
